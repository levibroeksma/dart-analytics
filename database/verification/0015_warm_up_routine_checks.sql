...
-- 0015 landed and resolve end to end: the Warm-Up exercise
-- template is a WARM_UP template with no game type, its
-- default_configuration holds five phases, and no
-- exercise_sessions row was left without an exercise type by
-- the backfill. The standalone Warm-Up routine 0015 once seeded
-- is gone (seed 0031); its absence is checked by
-- 0031_remove_default_routines_checks.sql.
--
...
INSERT INTO verification_results
SELECT '4',
    'the Warm-Up exercise template is WARM_UP with no game type',
    CASE WHEN count(*) = 1 THEN 'PASS' ELSE 'FAIL' END,
    format('found %s', count(*))
