-- ============================================================
-- Verification: 0025_score_threshold_seed_checks.sql
--
-- Proves against a live database what seed
-- 0025_score_threshold_exercise_type.sql can only claim locally:
-- the SCORE_THRESHOLD exercise type is published, its v1 ruleset
-- belongs to it, the system template pins that ruleset with the
-- rules document's locked threshold, and the template reaches
...
--
-- Usage:
--   psql "$DATABASE_URL" -f database/verification/0025_score_threshold_seed_checks.sql
--
...
SELECT '1',
    'SCORE_THRESHOLD exercise type exists and is published',
    CASE WHEN count(*) = 1 THEN 'PASS' ELSE 'FAIL' END,
    format('%s published row(s)', count(*))
FROM exercise_types
WHERE implementation_key = 'SCORE_THRESHOLD'
    AND is_published;

INSERT INTO verification_results
SELECT '2',
    'SCORE_THRESHOLD_V1 is version 1 of SCORE_THRESHOLD',
    CASE WHEN count(*) = 1 THEN 'PASS' ELSE 'FAIL' END,
    format('%s matching row(s)', count(*))
FROM exercise_ruleset_versions erv
    JOIN exercise_types et ON et.id = erv.exercise_type_id
WHERE erv.implementation_key = 'SCORE_THRESHOLD_V1'
    AND erv.version_number = 1
    AND et.implementation_key = 'SCORE_THRESHOLD';

INSERT INTO verification_results
SELECT '3',
    'the system template pins SCORE_THRESHOLD_V1 with the 65 threshold',
    CASE WHEN count(*) = 1 THEN 'PASS' ELSE 'FAIL' END,
...
WHERE et.implementation_key = 'SCORE_THRESHOLD'
    AND erv.implementation_key = 'SCORE_THRESHOLD_V1'
    AND t.is_system_template
    AND t.game_type_id IS NULL
    AND t.default_configuration = '{"threshold":65}'::jsonb;
...
WHERE exercise_type_key = 'SCORE_THRESHOLD'
