-- ============================================================
-- Verification: 0013_singles_training_v2_capability_checks.sql
--
-- Mirrors 0012_shanghai_v2_capability_checks.sql's shape,
-- re-scoped for the additive SINGLES_V2 rows appended to 0007_
...
-- before merge:
--
--   1. SINGLES_V2 + RECREATIONAL + DETAILED_DARTS resolved
--   2. SINGLES_V2 + ANALYTICS + VISUAL_BOARD resolved
...
-- SINGLES_V2's own additions.
--
-- Usage:
--   psql "$DATABASE_URL" -f database/verification/0013_singles_training_v2_capability_checks.sql
--
-- Expected: every result row reads PASS. Run only after
-- `npm run db:seed` has applied seeds/0007 and seeds/0013.
-- ============================================================
BEGIN;
...
-- ------------------------------------------------------------
-- Step 1: SINGLES_V2 + RECREATIONAL + DETAILED_DARTS resolved.
-- ------------------------------------------------------------
INSERT INTO verification_results
SELECT '1',
    'SINGLES_V2 / RECREATIONAL / DETAILED_DARTS resolves to a seeded row',
    CASE
    ...
    CASE
        ...
        WHEN rv.id IS NULL THEN 'no ruleset_versions row for SINGLES_V2'
...
WHERE rv.implementation_key = 'SINGLES_V2';

-- ------------------------------------------------------------
-- Step 2: SINGLES_V2 + ANALYTICS + VISUAL_BOARD resolved.
-- ------------------------------------------------------------
INSERT INTO verification_results
SELECT '2',
    'SINGLES_V2 / ANALYTICS / VISUAL_BOARD resolves to a seeded row',
    CASE
        WHEN c.ruleset_version_id IS NOT NULL THEN 'PASS'
        ELSE 'FAIL'
    END,
