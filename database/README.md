...
| `verification/0040_exercise_template_game_ruleset_checks.sql` | migration `0040` + seeds `0021`/`0022`: the pin column and its composite FK exist, a ruleset version of another game is rejected, a half-paired new row is rejected, seed `0021` backfilled Finishing to `TUOD_V1`, no system GAME template is left half-paired, both routine views expose `game_ruleset_version_key`, seed `0022`'s two templates are present and valid (8 checks) |
