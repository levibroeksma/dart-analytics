...
| `verification/0015_warm_up_routine_checks.sql` | seeds `0014`/`0015` resolve end to end: both exercise types, `WARM_UP_V1`, `EXERCISE_SECTION`, the Warm-Up exercise template is WARM_UP with no game type and five phases, no unbackfilled session (6 checks; the routine checks moved to `verification/0031_remove_default_routines_checks.sql` when seed `0031` removed that routine, D363) |
