...
Exercise engine files here are still `*.engine.module.ts` — `double-pattern`, `switching`, `switching-target-scoring`, `target-scoring` and `warm-up` under `exercises/`. The `GameEngine` contract does **not** apply to them.
