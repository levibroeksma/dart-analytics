...
- Never modify applied migrations (`0001`–`0041`); new schema change = new numbered migration + spec update. `0038` was committed unapplied under D336, but was applied on 2026-09-19, so no migration through `0039` is editable in place any more. `0040`/`0041` are likewise committed on this branch (D339/D342); no `DATABASE_URL` reaches this sandbox to confirm whether either has run anywhere, so treat both as equally non-editable rather than inferring "unapplied" from that silence. (2026-09-19; 0040 noted 2026-09-20; 0041 noted 2026-09-20)
