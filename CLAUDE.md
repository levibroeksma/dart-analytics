...
- Create a worktree with `git worktree add .worktrees/<name> -b <branch> origin/main`, then enter it with the `EnterWorktree` tool's `path` argument. Do **not** use `EnterWorktree`'s `name` argument: it hardcodes `.claude/worktrees/`, the retired path, and would violate the invariant above by construction. (2026-09-18, D319)
