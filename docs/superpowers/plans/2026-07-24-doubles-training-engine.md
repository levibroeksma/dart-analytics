...
**Architecture:** A pure reducer `applyDart(state, hit)` holds all progression/tracking rules and is unit-tested in isolation. A `DoublesTrainingEngine` class wraps it with the same call-site shape as `Bobs27Engine`/`SinglesTrainingEngine`, adding a history stack for undo. Everything lives in `app/src/modules/game/`, following the existing module-per-game-engine convention.
