...
Existing sessions were set under the old cross-origin cookie domain. After this deploys, the client asks the app's own origin instead, finds no cookie, and redirects to `/login`. Everyone gets logged out once. Harmless, one-time — not a regression.
