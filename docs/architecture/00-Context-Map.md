...
| `00-Context-Map.md` | This file — routing, packs, authority | canonical | ~12k |
...
| `app/src/modules/game/board-input.module.ts` | Visual board input, pure: `screenToBoard` (viewport px → board mm via the SVG's inverse screen CTM), `boardPxPerMm`, `magnifierPlacement` (handedness, edge flip, per-axis viewport clamp), `boardInput` press-drag-release state machine (D199, 2026-08-09) | canonical |
| `app/src/lib/game/board-input.data.ts` | Alpine/DOM bridge for the board: pointer events → controller, a fresh controller per press so a resize or rotation between gestures is picked up (2026-08-09) | canonical |
| `app/src/lib/game/session-mode-resolution.ts` | `resolveSessionModePair` (player settings + ruleset capability → the pair a new session is created with) and `engineInputMode` (stored `inputModeKey` → the engine's own mode), shared by both play pages (2026-08-09) | canonical |
...
| `app/src/components/ui/BoardMagnifier.astro` | Zoomed inset following the fingertip: a second clipped `DartBoard`, crosshair, live resolved read; `aria-hidden`, `fixed`, zoom scaled to the displayed board (D199, 2026-08-09) | canonical |
| `app/src/components/layout/games/BoardInputPanel.astro` | The board block both play pages mount: mode gate, `touch-none` pointer surface, magnifier `x-if` guard, "Dart not seen" action; declares no `x-data` of its own (2026-08-09) | canonical |
...
| `app/src/components/forms/AppModeForm.astro` | Profile-page mode picker — roving-tabindex radiogroup with a non-colour selection cue | canonical |
...
| `decisions/frontend/astro.md` | 17 decisions — .astro, component, prerender, routing, layout, cn(), props, frontmatter, PWA, manifest, icon, safe-area | canonical | ~2.8k |
