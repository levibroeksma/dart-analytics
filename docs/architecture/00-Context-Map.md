...
| `decisions/context-system.md` | 29 decisions — docs, context map, CLAUDE.md, skill, gate, check script, knowledge graph, CI, deploy, Prettier, format, husky | canonical | ~3.2k |
...
| `.github/workflows/graph.yml` | CI-owned graph freshness: rebuilds `graphify-out/graph.json` (`GRAPH_REFRESH_STRICT=1`) on every push to `main` and opens/updates a PR with the delta; on pull requests, posts a non-blocking delta comment via `scripts/graph-delta.py` without committing (2026-08-03) | canonical |
