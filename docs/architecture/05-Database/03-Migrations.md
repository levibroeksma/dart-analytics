...
- new `v_training_schedules` (schedule_id, player_id, name, is_active, updated_at, day_count — `count(*)::int`, cast for the same reason `routine_minutes` is: an uncast `bigint` reads back from node-postgres as a string, issue #538/D344) and `v_training_schedule_days` (schedule_id, player_id, schedule_name, is_active, day_of_week, routine_template_id, routine_name, routine_minutes)
...
Applied to the Neon dev branch on 2026-09-20, and `database/verification/0041_training_schedule_checks.sql` ran against it: ALL 9 CHECKS PASSED (one-active-per-player, both `day_of_week` bounds, duplicate weekday, routine-delete RESTRICT, both cascades, `routine_minutes`, `day_count`). <!-- 2026-09-20 -->
