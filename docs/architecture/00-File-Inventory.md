...
| `08-DartBot.md` | Simulated opponent architecture: seat layer plug-in, the play loop, delivery phases, board-geometry/engine-contract boundaries; phases 1–5 (throw engine, calibration, dictated strategy, seat admission, participant write path) are built — see `app/src/modules/dartbot/`, `app/src/lib/game/guest-list.ts`/`session-mode-resolution.ts`, `app/src/services/session.service.ts`; phase 6+ (play loop, ghost mode, 501 opponent) remains design-only (2026-09-01) | canonical | ~22.9k |
