...
| `decisions/database.md` | 21 decisions — schema, migration, table, column, constraint, index, view, Neon, seed (D257 general statistics views, 2026-09-06; D263 exercise-type discriminator, D266 exercise_templates.exercise_type_id backfill shape, 2026-09-10; D284 training gameplay in the read model, 2026-09-16; D288 CI-applied production migrations behind a rehearsal, D290 schema.ts the only committed introspect artifact, D291 uq_sessions_single_active keys on COALESCE(game_type_id, exercise_type_id), 2026-09-17) | canonical | ~6.8k |
| `decisions/api.md` | 40 decisions — endpoint, contract, envelope, auth, middleware, idempotency, batch, Worker, player settings (D283 training activity read player-scoped, terminal transition guarded in the UPDATE; D285 active-session conflict detection walks the error `cause` chain, 2026-09-16; D292 starting a routine abandons the player's previous one, 2026-09-17) | canonical | ~8.4k |
