...
| `08-DartBot.md` | Simulated opponent architecture: seat layer plug-in, the play loop, delivery phases, board-geometry/engine-contract boundaries; phases 1–6 (throw engine, calibration, dictated strategy, seat admission, participant write path, play loop) are built — see `app/src/modules/dartbot/`, `app/src/lib/game/guest-list.ts`/`session-mode-resolution.ts`/`play-lifecycle.ts`/`bobs27-play.data.ts`, `app/src/services/session.service.ts`; phase 7+ (ghost mode, 501 opponent) remains design-only (2026-09-01) | canonical | ~23.1k |
