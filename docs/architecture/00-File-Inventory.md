...
| `11-Neon-Integration.md` | Neon topology, branches, dbmate/drizzle workflow; `env:dev`/`env:prod` PUBLIC_ mirror; per-branch trusted origins (D172, 2026-07-29); CI applies production migrations behind a Neon-branch rehearsal, `:ci` script variants, required secrets (D288, 2026-09-17) | canonical | ~2.2k |

## API (`06-API/`) and Frontend (`07-Frontend/`)

| File | Answers | Status | ~Tokens |
| ---- | ------- | ------ | ------- |
| `06-API/00-Overview.md` | Frozen v1 API baseline: runtime, routes, auth, envelopes; `/api/auth/*` same-origin proxy (D172, 2026-07-29); `/api/players/me/settings` routed (D195, 2026-08-08); `GET /api/statistics/overview` routed (2026-09-06) | canonical | ~3.1k |
...
| `app/src/modules/exercise/interfaces.ts` | `ExerciseEngine<TState>`/`ExerciseEngineFactory` (phase-driven, `advance()`) plus `DartExerciseEngine<TState>`/`DartExerciseEngineFactory` (dart-driven, `record(observation)`) — siblings, neither built on the other nor on `GameEngine` (D264; `DartExerciseEngine` added D267, 2026-09-11) | canonical |
| `app/src/modules/exercise/dart-engine.registry.ts` | `ExerciseRulesetVersionKey` → `DartExerciseEngineFactory` lookup, mirroring `engine.registry.ts` for the dart-driven engine family (`SWITCHING_V1`/`DOUBLE_PATTERN_V1`) (2026-09-11) | canonical |
| `app/src/modules/exercise/solo-participant.module.ts` | `SOLO_PARTICIPANT_REF` — the fixed `turns` participant every solo exercise session logs under; training routines have no seat/opponent concept to derive one from (2026-09-11) | canonical |
| `app/src/modules/exercise/switching.engine.module.ts` | `SwitchingEngine`/factory for `SWITCHING_V1`: cycles a fixed `targets: number[]` list dart by dart, `scoring.{single,double,treble}` for a hit on the current target, 0 for anything else including a wrong number ("outside" by omission); exported for `expireTimer()` access (2026-09-11) | canonical |
| `app/src/modules/exercise/double-pattern.engine.module.ts` | `DoublePatternEngine`/factory for `DOUBLE_PATTERN_V1`: cycles a fixed `patterns: number[][]` list, one point per hit double, nothing else scores; exported for `expireTimer()` access (2026-09-11) | canonical |
...
| `app/src/services/exercise-rulesets/switching/switching.validator.ts` | `switchingValidator` for `SWITCHING_V1` — parses `targets`/`scoring` only; no mode pair, no dart-row bound (the §7 sixty-minute cap lives in `modules/training/routine-duration.module.ts`) (2026-09-11) | canonical |
...
| `decisions/database.md` | 22 decisions — schema, migration, table, column, constraint, index, view, Neon, seed (D257 general statistics views, 2026-09-06; D263 exercise-type discriminator, D266 exercise_templates.exercise_type_id backfill shape, 2026-09-10; D284 training gameplay in the read model, 2026-09-16; D288 CI-applied production migrations behind a rehearsal, D290 schema.ts the only committed introspect artifact, D291 uq_sessions_single_active keys on COALESCE(game_type_id, exercise_type_id), D295 exercise templates pin their exercise ruleset version, 2026-09-17) | canonical | ~6.8k |
| `decisions/api.md` | 41 decisions — endpoint, contract, envelope, auth, middleware, idempotency, batch, Worker, player settings (D283 training activity read player-scoped, terminal transition guarded in the UPDATE; D285 active-session conflict detection walks the error `cause` chain, 2026-09-16; D292 starting a routine abandons the player's previous one, D296 startTraining validates every step's configuration, 2026-09-17) | canonical | ~8.4k |
...
| `decisions/testing.md` | 7 decisions — test, TDD, Vitest, mock, coverage, change-set test gate, verification-script parity gate | canonical | ~1.8k |
