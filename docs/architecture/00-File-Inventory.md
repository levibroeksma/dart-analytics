...
| `08-DartBot.md` | Simulated opponent architecture: seat layer plug-in, the play loop, delivery phases, board-geometry/engine-contract boundaries; phases 1–7 (throw engine, calibration, dictated strategy, seat admission, participant write path, play loop, `X01Strategy` + 501 opponent mode) are built — see `app/src/modules/dartbot/`, `app/src/lib/game/guest-list.ts`/`session-mode-resolution.ts`/`play-lifecycle.ts`/`bobs27-play.data.ts`/`five-oh-one-play.data.ts`/`shanghai-play.data.ts`/`singles-training-play.data.ts`, `app/src/services/session.service.ts`; phase 8+ (ghost mode) remains design-only; the play loop now also covers Shanghai and Singles Training (2026-09-03) alongside Bob's 27 — Around the Clock/Doubles Training remain admitted but unwired (`FINDINGS.md` F61) (2026-09-02; Shanghai wiring 2026-09-03; Singles Training wiring closes F62, 2026-09-03) | canonical | ~24.4k |
