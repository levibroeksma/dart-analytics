...
| `decisions/database.md` | 18 decisions — schema, migration, table, column, constraint, index, view, Neon, seed (D263 exercise-type discriminator, D266 exercise_templates.exercise_type_id backfill shape, 2026-09-10; D288 uq_sessions_single_active keys on COALESCE(game_type_id, exercise_type_id), 2026-09-17) | canonical | ~5.7k |
| `decisions/api.md` | 40 decisions — endpoint, contract, envelope, auth, middleware, idempotency, batch, Worker, player settings (D285 active-session conflict detection walks the error `cause` chain, 2026-09-16; D289 starting a routine abandons the player's previous one, 2026-09-17) | canonical | ~8.4k |
