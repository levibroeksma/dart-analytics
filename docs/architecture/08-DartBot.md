...
| **The play loop** — trigger, `botThrowing` guard, post-delay re-check, `undoToActiveSeat()`, QUICK_SCORE scratch-engine fold | **shipped on all eight `PER_SEAT` rulesets** — phase 6 (2026-09-01) built the mechanism generically in `play-lifecycle.ts` and wired it onto Bob's 27, the one `VISUAL_BOARD`-only ruleset `RULESET_DARTBOT` admitted at the time; Shanghai and Singles Training followed the identical wiring 2026-09-03, then 121, TUOD and Score Training the same day, and Around the Clock and Doubles Training 2026-09-16 (issue #283). A seated `DARTBOT` opponent now actually throws in all eight; one undo press returns to the human across any number of consecutive bot turns. `501_V1` gets the same mechanism through its own `SHARED`-stage `X01Strategy` row below. | `app/src/lib/game/play-lifecycle.ts`, `app/src/lib/game/bobs27-play.data.ts`, `app/src/lib/game/shanghai-play.data.ts`, `app/src/lib/game/singles-training-play.data.ts`, `app/src/lib/game/one-twenty-one-play.data.ts`, `app/src/lib/game/tuod-play.data.ts`, `app/src/lib/game/score-training-play.data.ts`, `app/src/lib/game/around-the-clock-play.data.ts`, `app/src/lib/game/doubles-training-play.data.ts` |
...
The default opponent mirrors the user's own fitted profile plus a chosen challenge offset, presented as **a level number on the same 1–15 scale the setup slider uses** (D-D). No tier name and no expected average is shown: a three-dart average is meaningless in Around the Clock, and a tier name invented before D-E fits the curve would have to be renamed once it is. The population prior behind this curve, its anchor levels, and the scale/exponent that shaped each pass are tracked in `08-DartBot-Anchor-Log.md` (D-L, D-N) rather than here — a future re-anchor updates that file, not this paragraph.
...
**Then done:** `LEVEL_SKILL_TABLE` is now refit against this prior (D-L, below). `fitProfile()` itself (phase 10) is still not built. The level-picker average/checkout UI no longer waits on it — it shipped against this refit's output (D-M, below).
...
## Resolved: D-M level-select stats

**D-M — level-select stats.** The level-picker average/checkout UI deferred above is built: `OpponentChooserModal.astro`'s level step shows a three-dart-average band and a checkout-% band per level, sourced from `allLevelSelectStats()` (`app/src/modules/dartbot/level-select-stats.module.ts`) — the sole access point, so a later swap to a live DB lookup or on-the-fly compute changes only that function's body. Backed today by a table precomputed by `app/scripts/dartbot-level-select-stats.ts` (25th/75th percentile per level, `N = 5000`, seed `800000 + level`) (`docs/superpowers/specs/2026-09-04-dartbot-level-select-stats-design.md`, 2026-09-04).

## Resolved: D-N level-curve recalibration

**D-N — `LEVEL_SKILL_TABLE` recalibration.** D-L's single anchor (level 6, D-E) plus a level-1-only search left level 15 degenerate: `sigmaAlongMm`/`sigmaAcrossMm` so tight the bot landed treble 20 almost every dart, producing a three-dart-average band clipped at the 180 ceiling and a ~96% checkout rate — not a plausible "elite" opponent. A human supplied replacement guardrails at both ends of the scale instead of one: level 1 three-dart average 20–30 / checkout% 5–20%, level 15 average 90+ / checkout% 45–80%.

Level 15's pair is jointly satisfiable and was searched directly. Level 1's is not: in this engine, average and single-dart double-hit rate move together on one curve regardless of how spread is split between radial (`sigmaAlongMm`) and angular (`sigmaAcrossMm`) precision — checkout 5%+ never appears before average is already past ~40, well outside the stated 20–30 ceiling. Told this, the human chose a compromise anchor (~32–37 average, ~3–4% checkout) over forcing either literal band. Level 6 is untouched — still the exact D-E row.

`app/scripts/dartbot-level-curve-recalibration.ts` binary-searches a single uniform spread scale per anchor (level 1 against the compromise average band, level 15 against its checkout band, with average checked only as a floor), then fills the other twelve levels with a two-segment log-space interpolation (1→6, 6→15) that keeps the pre-recalibration curve's relative shape between whichever pair of anchors brackets a given level — the same shape-preserving principle D-L's single power-law rescale used, generalized to two segments. `decisionQuality`, `bedOffsetMm`, `bounceOutRate`, `deflectionRadiusMm` and `covarianceRotationDegrees` are untouched, as in D-L.

The anchor levels, the guardrails, the found scales and the measured values are recorded in `08-DartBot-Anchor-Log.md` rather than restated here. D-M's level-select stats table was regenerated against the recalibrated curve in the same task — level 15's average band is no longer clipped at 180.
