...
| **The play loop** — trigger, `botThrowing` guard, post-delay re-check, `undoToActiveSeat()`, QUICK_SCORE scratch-engine fold | **shipped** — phase 6 (2026-09-01) built the mechanism generically in `play-lifecycle.ts` and wired it onto Bob's 27, the one `VISUAL_BOARD`-only ruleset `RULESET_DARTBOT` already admits. A seated `DARTBOT` opponent now actually throws; one undo press returns to the human across any number of consecutive bot turns. The QUICK_SCORE fold is proven at the engine level (`scoreTrainingEngineFactory`) but has no live consumer yet — no `QUICK_SCORE` ruleset admits a bot until phase 7 adds the flag to `501_V1` | `app/src/lib/game/play-lifecycle.ts`, `app/src/lib/game/bobs27-play.data.ts` |
...
└── strategy/
    ...
    ├── scoring.strategy.module.ts      # shared by 501 and Score Training (D-G)
...
```ts
// modules/dartbot/interfaces.ts — the module declares
...
`opponentRemaining` used to have no reliable source: `FiveOhOneState` was a flat object mixing per-player, per-side and per-session fields. It now splits into `seats[]` (a `remainingScore` per seat), `sides[]` (`legsWon` per side) and `status`, so the adapter reads the opponent's number straight off `state().seats` instead of the page tracking it.
