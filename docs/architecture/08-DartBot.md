...
| **The play loop** — trigger, `botThrowing` guard, post-delay re-check, `undoToActiveSeat()`, QUICK_SCORE scratch-engine fold | **shipped on Bob's 27 and Shanghai** — phase 6 (2026-09-01) built the mechanism generically in `play-lifecycle.ts` and wired it onto Bob's 27, the one `VISUAL_BOARD`-only ruleset `RULESET_DARTBOT` admitted at the time; Shanghai followed the identical wiring 2026-09-03. A seated `DARTBOT` opponent now actually throws in either; one undo press returns to the human across any number of consecutive bot turns. Around the Clock and Doubles Training are admitted in `RULESET_DARTBOT` but their play pages never received this wiring — `FINDINGS.md` F61. | `app/src/lib/game/play-lifecycle.ts`, `app/src/lib/game/bobs27-play.data.ts`, `app/src/lib/game/shanghai-play.data.ts` |
