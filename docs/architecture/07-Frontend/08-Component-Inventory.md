...
| `SinglePlayerDisplay.astro` | Score-or-target panel with a `progress` slot | `score`, `target`, `isTarget`, `size` (`lg`/`sm`) |
...
| `SplitScoreboardHalf.astro` | One seat's column inside `SplitScoreboard`: name/active-dot, optional leg-wins pill, compact `SinglePlayerDisplay` (`size="sm"`), optional checkout chips, optional leg dot pager | `nameExpr`, `activeExpr`, `scoreExpr`, `isTarget`, `legsExpr`, `legsToWinExpr`, `checkoutExpr`; default slot renders in the progress region |
