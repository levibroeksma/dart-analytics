...
### F23 — `foldSinglesTrainingState` is exported but has no consumer outside its own module
Status: Open · Found: 2026-08-22 · Task: claude/guest-player-x01-architecture-m8ia8v
Claim: Task 17's brief exports `foldSinglesTrainingState` alongside `SinglesTrainingEngine`, mirroring every other multi-seat engine's own exported fold function in this plan; its doc comment says it "mirrors `foldAroundTheClockState`"
Evidence: `npx fallow`'s "Unused exports" check flags `app/src/modules/game/singles-training.engine.module.ts:135 foldSinglesTrainingState` — it is called only internally by `SinglesTrainingEngine.deriveState()` in the same file. `app/src/lib/game/singles-training-play.data.ts` (Task 18's file, untouched by this task) does not import it; it still reads the pre-1v1 flat `SinglesTrainingState` shape directly off `this.engine.state()`. The identical class of finding is already logged for Around The Clock's own `foldAroundTheClockState` at F17, whose evidence shows that engine's frontend task wired `state()` through `this.engine?.state()` rather than importing the fold function directly — leaving it dead even after its own frontend task landed, not just in the interim
Impact: `npx fallow` (part of `npm run validate:app`'s completion bar) fails on this dead export today, exactly as it already does for Around The Clock (F17 — `dead exports 0.3% (2 of 622)` after this task, both `foldAroundTheClockState` and `foldSinglesTrainingState`). If Task 18 wires `singles-training-play.data.ts`'s `state()` the same `this.engine?.state()` way Around The Clock's and Bob's 27's frontends do — rather than importing `foldSinglesTrainingState` directly, the way `one-twenty-one-play.data.ts` does for `foldOneTwentyOneState` — this export stays dead permanently, not just until Task 18 lands
Proposed: when Task 18 wires `singles-training-play.data.ts`, either call `foldSinglesTrainingState` directly from its `state()` accessor (matching the `one-twenty-one`/`foldOneTwentyOneState` precedent) or, if `this.engine?.state()` is kept for consistency with the sibling frontends, drop the `export` keyword so the function becomes a plain module-private helper — either resolves this and F17 together, since both share the same root cause; touches `singles-training.engine.module.ts` and/or `singles-training-play.data.ts`, outside this task's designated files
