...
### F25 — `npx fallow` still fails the whole branch, now also on a new `match-outcome.module.ts` health-threshold breach
Status: Open · Found: 2026-08-22 · Task: claude/guest-player-x01-architecture-m8ia8v
Claim: `npm run validate:app`'s completion bar (`app/CLAUDE.md`'s Validation Standard Procedure) requires every step, `npx fallow` included, to exit zero
Evidence: `cd app && npx fallow` (run for this task's Step 5 full validation, with the DB-dependent steps ahead of it in the `validate:app` chain skipped — see this task's completion report for why): `✗ 1 above threshold · 4475 analyzed · maintainability 91.9 (good)` / `Failed: dead-code (1 issues), dupes (95 clone groups), health (1 above threshold): start with src/modules/game/match-outcome.module.ts`. The dead-code failure was F17 — fixed 2026-08-22 by the whole-plan review-fix pass (`foldAroundTheClockState` made module-private), so `npx fallow` now reports `Failed: dupes (95 clone groups), health (1 above threshold)` and the dead-code check passes. The duplication failure overlaps F16, still logged and unfixed by design (F16: "not fixed mid-plan... after all seven engines land, run fallow once across the whole branch and decide"). The health failure is new: fallow's refactoring-targets section flags `app/src/modules/game/match-outcome.module.ts` as `11.6 pri:23.1 ... high impact · effort:medium · confidence:medium Split high-impact file (47 LOC), 9 dependents amplify every change` — its three winner functions are now imported by all eight 1v1 engines (`around-the-clock`, `bobs27`, `doubles-training`, `one-twenty-one`, `score-training`, `shanghai`, `singles-training`, `tuod`), and that fan-in (8 engines + 1 test file = fallow's own "9 dependents" figure) crosses fallow's CRAP/health threshold for a 47-line file
Impact: Task 21 (this task) is the point F16 named for deciding whether the plan's duplication is acceptable, but Task 21's own scope is docs + final validation only — no source changes — so neither F16's deferred decision nor this health flag could be resolved in that pass (F17's dead export since has been, separately). `npm run validate:app` cannot show a fully clean run for this branch until a source-touching follow-up task addresses the remaining two
Proposed: a follow-up task (not this one) should: (1) make F16's deferred call now that all seven engines have landed — accept the seven-way `MultiSeatState` conversion's duplication as an intentional cost, or extract a shared per-engine scaffold; (2) decide whether `match-outcome.module.ts`'s 9-importer fan-in genuinely warrants a split, or whether a `.fallowrc.jsonc` threshold exception fits better for a small, intentionally-shared pure-function module
