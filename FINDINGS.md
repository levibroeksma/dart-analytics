...
### F24 — `foldDoublesTrainingState` is exported but has no consumer outside its own module
Status: Open · Found: 2026-08-22 · Task: claude/guest-player-x01-architecture-m8ia8v
Claim: Task 19's brief exports `foldDoublesTrainingState` alongside `DoublesTrainingEngine`, mirroring every other multi-seat engine's own exported fold function in this plan; its doc comment says it "mirrors `foldSinglesTrainingState`"
Evidence: `npx fallow`'s "Unused exports" check flags `app/src/modules/game/doubles-training.engine.module.ts:147 foldDoublesTrainingState` — it is called only internally by `DoublesTrainingEngine.deriveState()` in the same file. `app/src/lib/game/doubles-training-play.data.ts` (Task 20's file, untouched by this task) does not import it; it still reads the pre-1v1 flat `DoublesTrainingState` shape directly off `this.engine.state()`, which now fails to type-check until Task 20 lands. Same root cause as the already-open F17 (`foldAroundTheClockState`); F23, the identical finding for `foldSinglesTrainingState`, was closed when Singles Training's own frontend task wired its play page to call the fold function directly
Impact: `npx fallow` (part of `npm run validate:app`'s completion bar) fails on this dead export today. Whether it stays dead past Task 20 depends on how that task wires `doubles-training-play.data.ts`'s `state()` accessor — importing `foldDoublesTrainingState` directly (the `one-twenty-one`/F23-resolution precedent) closes this finding; keeping `this.engine?.state()` (the Around the Clock/F17 precedent) leaves it open
Proposed: when Task 20 wires `doubles-training-play.data.ts`, call `foldDoublesTrainingState` directly from its `state()` accessor, matching how Singles Training's frontend task resolved F23 — touches `doubles-training-play.data.ts`, outside this task's designated files
